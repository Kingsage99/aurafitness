    ...
    <>
      ...
      <div style={{ padding: '8px 22px 0', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <button style={{ width: 40, height: 40, borderRadius: 12, border: NB_BORDER, background: NB.magenta, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: hardShadow(3) }}>
      ...
      <div style={{ padding: '12px 22px 0', display: 'flex', gap: 8, flexShrink: 0 }}>
        {[['feed', 'Activity feed'], ['leaderboard', 'Leaderboard']].map(([id, label]) => (
          <button
            ...
            style={{
              flex: 1, height: 42, border: `2.5px solid ${NB.ink}`, borderRadius: 12,
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px 22px 0' }}>
        {activeTab === 'feed' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {FEED.map((item, i) => (
              <div key={i} style={{ border: NB_BORDER, borderRadius: 16, boxShadow: hardShadow(3), background: NB.white, padding: '14px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 10 }}>
                  <div style={{ width: 40, height: 40, borderRadius: 11, border: `2px solid ${NB.ink}`, background: item.color, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button
                    ...
                    style={{
                      ...
                      padding: '6px 12px', border: `1.5px solid ${NB.ink}`, borderRadius: 10,
                  ...
                  {item.comment && (
                    <button style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', border: `1.5px solid ${NB.ink}`, borderRadius: 10, background: NB.white, fontSize: 12, fontWeight: 700, color: NB.ink, cursor: 'pointer' }}>
        ...
        {activeTab === 'leaderboard' && (() => {
          ...
          return (
            <div>
              <div style={{ border: NB_BORDER, borderRadius: 18, boxShadow: hardShadow(4), background: NB.lavender, padding: '16px', marginBottom: 16 }}>
                <div style={{ fontFamily: NB.fontMono, fontSize: 10, fontWeight: 800, color: NB.ink, letterSpacing: 1, marginBottom: 8, textTransform: 'uppercase' }}>Weekly Leaderboard</div>
                <div style={{ fontSize: 13, color: NB.ink }}>Top 3 get gems bonus! 💎</div>
              </div>
                ...
                {board.map((entry) => {
                  ...
                  return (
                    <div
                      key={entry.name}
                      style={{
                        border: `2.5px solid ${NB.ink}`, borderRadius: 14, padding: '12px 16px',
                    ...
                    >
                      <div style={{
                        width: 32, height: 32, borderRadius: 10, border: `2px solid ${NB.ink}`, flexShrink: 0,
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                          ...
                          <span style={{ fontSize: 10, fontWeight: 800, color: rank.color === '#fff' ? NB.ink : rank.color, background: rank.bg, border: `1.5px solid ${NB.ink}`, borderRadius: 6, padding: '1px 6px' }}>{rank.label}</span>
                        </div>
